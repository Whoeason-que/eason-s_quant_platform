        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                if data is not None:
                    ...
                    # 新增清单数据收集
                    safe_name = "".join(c if c.isalnum() else "_" for c in company)
                    manifest_data["assets"].append({"name": company,})
                    logging.info(f"成功获取 {company} 数据 ({len(data)} 条)")
            except Exception as e:
                logging.error(f"处理 {company} 时发生异常: {str(e)}")
    # 修改后的清单文件生成（删除原有generate_manifest函数）
    try:
        manifest_data["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(MANIFEST_PATH, 'w', encoding='utf-8') as f:
            yaml.dump(manifest_data, f, allow_unicode=True)
            logging.info(f"数据清单已生成: {MANIFEST_PATH}")
    except Exception as e:
        logging.error(f"生成数据清单失败: {str(e)}")
